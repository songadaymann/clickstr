...
describe("StupidClicker", function () {
  let stupidClicker;
  let clickToken;
  let owner;
  let player1;
  ...
  beforeEach(async function () {
    ...
    // Deploy StupidClicker with season params (no NFT contract for basic tests)
    const StupidClicker = await ethers.getContractFactory("StupidClicker");
    stupidClicker = await StupidClicker.deploy(
  ...
  describe("Deployment", function () {
    ...
    it("should accept custom initial difficulty", async function () {
      ...
      const StupidClicker = await ethers.getContractFactory("StupidClicker");
      const customClicker = await StupidClicker.deploy(
        await clickToken.getAddress(),
        TOTAL_EPOCHS,
        EPOCH_DURATION,
        customDifficulty,
    ...
    it("should scale TARGET_CLICKS_PER_EPOCH based on epoch duration", async function () {
      ...
      const StupidClicker = await ethers.getContractFactory("StupidClicker");
      const halfDayClicker = await StupidClicker.deploy(
        await clickToken.getAddress(),
        TOTAL_EPOCHS,
        halfDayDuration,
        DEFAULT_INITIAL_DIFFICULTY,
    ...
    it("should store achievement NFT address", async function () {
      ...
      const StupidClickerNFT = await ethers.getContractFactory("StupidClickerNFT");
      const nft = await StupidClickerNFT.deploy(owner.address, "https://test.com/");

      const StupidClicker = await ethers.getContractFactory("StupidClicker");
      const clickerWithNFT = await StupidClicker.deploy(
        await clickToken.getAddress(),
        TOTAL_EPOCHS,
        EPOCH_DURATION,
        DEFAULT_INITIAL_DIFFICULTY,
  ...
  describe("Submitting Clicks", function () {
    ...
    it("should revert if game not started", async function () {
      const StupidClicker = await ethers.getContractFactory("StupidClicker");
      const newClicker = await StupidClicker.deploy(
        await clickToken.getAddress(),
        TOTAL_EPOCHS,
        EPOCH_DURATION,
        DEFAULT_INITIAL_DIFFICULTY,
  ...
  describe("NFT Bonus System", function () {
    ...
    beforeEach(async function () {
      // Deploy NFT contract
      const StupidClickerNFT = await ethers.getContractFactory("StupidClickerNFT");
      nftContract = await StupidClickerNFT.deploy(owner.address, "https://test.com/");

      // Deploy clicker with NFT support
      const StupidClicker = await ethers.getContractFactory("StupidClicker");
      clickerWithNFT = await StupidClicker.deploy(
        await clickToken.getAddress(),
        TOTAL_EPOCHS,
        EPOCH_DURATION,
        DEFAULT_INITIAL_DIFFICULTY,
